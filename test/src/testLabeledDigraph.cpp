...
/**
 * Test `LabeledDigraph<...>::getConditionalFPTMomentsFromRecurrence()`. 
 */
template <typename T, typename U>
void testGetConditionalFPTMomentsFromRecurrence(LabeledDigraph<T, U>* graph,
                                                const int num_nodes,
                                                std::vector<std::string>& node_ids, 
                                                std::unordered_set<int>& terminal_idx, 
                                                const Ref<const Matrix<U, Dynamic, Dynamic> >& laplacian, 
                                                const double eps)
{
    ...
    // Compute the conditional FPT moments, for r = 0, ..., 10, with the 
    // Chebotarev-Agaev recurrence and check that they satisfy the corresponding
    // linear equation 
    Matrix<U, Dynamic, Dynamic> probs = graph->template getSplittingProbsFromSolver<T, U>();
    for (int r = 0; r < 11; ++r)
    {
        Matrix<U, Dynamic, Dynamic> fpts = graph->template getConditionalFPTMomentsFromRecurrence<U>(r);
...
TEST_CASE("Tests for getConditionalFPTMomentsFromRecurrence()")
{
    typedef PreciseType T; 
    ...
    for (int i = 0; i < terminal_node_graph_funcs.size(); ++i)
    {
        auto result = terminal_node_graph_funcs[i](); 
        LabeledDigraph<T, T>* graph = std::get<0>(result); 
        testGetConditionalFPTMomentsFromRecurrence<T, T>(
            graph, std::get<1>(result), std::get<2>(result), std::get<8>(result),
            std::get<6>(result), 1e-50
        );
        delete graph;
